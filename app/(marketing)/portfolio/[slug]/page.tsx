  ...
  return (
    <>
      ...
      <Section className="border-t border-[var(--color-line)]" size="sm">
        <Container>
          <Stagger className="grid grid-cols-2 md:grid-cols-4 gap-[1px] bg-[var(--color-line)] border border-[var(--color-line)]">
            {c.metrics.map((m) => (
              <StaggerItem
                key={m.label}
                className="bg-[var(--color-paper)] p-6 md:p-8 flex flex-col justify-between min-h-[160px]"
              >
        ...
        <Container>
          <div className="grid grid-cols-1 md:grid-cols-12 gap-x-8 gap-y-16">
            <Reveal as="div" className="md:col-span-4 md:sticky md:top-[100px] md:self-start">
              ...
              <p className="mt-4 h3 max-w-[16ch]">Three passes at the shape.</p>
              ...
              {c.narrative.map((n, i) => (
                <Reveal key={n.heading} delay={i * 0.08}>
                  <article>
                    ...
                    <h3 className="mt-3 font-[family-name:var(--font-display)] text-[clamp(1.5rem,1.1rem+1.6vw,2.5rem)] leading-[1.1] tracking-[-0.02em] text-[var(--color-ink)] max-w-[24ch]">
                      {n.heading}
                    </h3>
                    <p className="mt-5 text-[var(--color-muted)] leading-[1.7] max-w-[62ch]">
                      {n.body}
                    </p>
                  </article>
                </Reveal>
            ...
            <Reveal>
              ...
              <ul className="mt-4 flex flex-wrap gap-x-3 gap-y-3">
                {c.stack.map((t) => (
                  <li
                    key={t}
                    className="kicker uppercase px-3 py-1 border border-[var(--color-line-strong)] rounded-[var(--radius-sm)]"
                  >
                    {t}
                  </li>
                ))}
