...
export default async function ServicePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <>
        ...
        <Container>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-16">
            <Reveal as="div">
              ...
              <h2 className="mt-4 h3 max-w-[20ch]">What you take away.</h2>
              <ul className="mt-8 border-t border-[var(--color-line-strong)]">
                {s.outcomes.map((o, i) => (
                  <li
                    key={i}
                    className="py-4 border-b border-[var(--color-line)] flex items-baseline gap-4"
                  >
                    <span className="num text-xs text-[var(--color-gold-2)] w-6 pt-1">
            ...
            <Reveal delay={0.1} as="div">
              ...
              <h2 className="mt-4 h3 max-w-[20ch]">What we ship.</h2>
              <ul className="mt-8 border-t border-[var(--color-line-strong)]">
                {s.deliverables.map((d, i) => (
                  <li
                    ...
                    className="py-4 border-b border-[var(--color-line)] flex items-baseline gap-4"
                  >
                    <span className="num text-xs text-[var(--color-muted)] w-6 pt-1">
                      {String(i + 1).padStart(2, "0")}
                    </span>
                    <span className="text-[var(--color-ink)] leading-relaxed">{d}</span>
                  </li>
                ))}
              </ul>
            </Reveal>
      ...
      {/* Method */}
      <Section className="border-t border-[var(--color-line)]">
        <ProcessRail kicker="Method" heading={`How a ${s.title.toLowerCase()} engagement runs.`} steps={s.method} />
      </Section>
          ...
          <Container wide>
            <div className="flex items-baseline justify-between mb-10">
              <div>
                ...
                <h2 className="mt-4 h2 max-w-[24ch]">
                  What this practice has shipped recently.
                </h2>
              </div>
              <Link href="/portfolio" className="link-underline kicker uppercase hidden md:inline">
                All work →
              </Link>
            </div>
            <Stagger className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-12">
              {relatedCases.map((c) => (
        ...
        <Container>
          <div className="grid grid-cols-1 md:grid-cols-12 gap-x-8 gap-y-10">
            <Reveal as="div" className="md:col-span-4">
              ...
              <h2 className="mt-4 h2 max-w-[16ch]">Questions we hear on the first call.</h2>
            </Reveal>
      ...
      {related.length > 0 && (
        <Section size="sm" className="border-t border-[var(--color-line)]">
          <Container>
            ...
            <ul className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-x-8 gap-y-4">
              {related.map((r) => (
                <li key={r.slug}>
                  <Link
                    href={`/services/${r.slug}`}
                    className="group block border-t border-[var(--color-line-strong)] pt-4 hover:border-[var(--color-ink)] transition-colors"
                  >
                    ...
                    <p className="mt-1 text-[var(--color-muted)] text-sm">{r.kicker}</p>
                  </Link>
                </li>
              ))}
            </ul>
