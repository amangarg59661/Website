              ...
              {articleCategories.map((c) => (
                <li key={c}>
                  <Link
                    ...
                    className={`link-underline ${c === cat ? "text-[var(--color-ink)] font-medium" : "text-[var(--color-muted)]"}`}
                  >
                    {c}
                  </Link>
                </li>
      ...
      <Section className="border-t border-[var(--color-line)]">
        <Container>
          <div className="border-t border-[var(--color-line)]">
            {list.length === 0 && (
              <p className="lede py-16">Nothing in this category yet.</p>
            )}
            {list.map((a, i) => (
