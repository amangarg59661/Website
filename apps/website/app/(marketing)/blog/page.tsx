...
export default function BlogPage() {
  ...
  const list = useMemo(() => articles.filter((a) => cat === "All" || a.category === cat), [cat]);
  ...
  return (
    <>
      <PageIntro
        ...
        lede={
          <>
            Long-form essays on engineering, AI, design, and marketing — the pieces we would publish
            even without a website. Read one on a Sunday.
          </>
        }
        aside={
          <div>
          ...
          <div className="mb-8 flex flex-wrap items-center gap-3">
            ...
            {articleCategories.map((c) => (
              <button
                ...
                aria-pressed={cat === c}
                className={cn(
                  "kicker relative px-3 py-1 text-[var(--color-muted)] uppercase transition-colors hover:text-[var(--color-ink)]",
                  cat === c &&
