  ...
  return (
    <>
      <PageIntro
        ...
        lede={
          <>
            Elite Digital is a boutique studio. Forty-six people, three offices, one operating model
            — small teams of senior operators, led by partners, working under the client&apos;s
            constraints instead of around them. We started in Bengaluru in {site.founded}. We remain
              ...
              <ul className="mt-2 space-y-1">
                {site.offices.map((o) => (
                  <li key={o.city} className="text-[var(--color-ink)]">
                    {o.city}, {o.country}
                    <span className="ml-2 text-sm text-[var(--color-muted)]">{o.role}</span>
                  </li>
                ))}
              </ul>
            </div>
        ...
        <Container>
          <div className="grid grid-cols-1 gap-x-8 gap-y-14 md:grid-cols-12">
            <Reveal as="div" className="md:col-span-4">
              ...
              <h2 className="h2 mt-4 max-w-[16ch]">Four principles, one operating manual.</h2>
              <p className="lede mt-6 max-w-[42ch]">
                Every decision we make — hiring, pricing, retainer terms, the way we run Fridays —
                comes back to these.
              </p>
            </Reveal>
            <Stagger className="grid grid-cols-1 gap-x-8 gap-y-12 md:col-span-8 md:grid-cols-2">
              {beliefs.map((b) => (
                <StaggerItem key={b.n}>
                  <div className="border-t border-[var(--color-line-strong)] pt-6">
                    <div className="flex items-baseline gap-4">
                      <span className="num w-6 text-sm text-[var(--color-muted)]">{b.n}</span>
                      <h3 className="font-[family-name:var(--font-display)] text-[clamp(1.25rem,1rem+1vw,1.65rem)] leading-[1.15] tracking-[-0.02em]">
                        {b.t}
                      </h3>
                    </div>
                    <p className="mt-4 max-w-[52ch] pl-10 leading-[1.7] text-[var(--color-muted)]">
                      {b.b}
                    </p>
                  </div>
                </StaggerItem>
      ...
      <Section>
        <ProcessRail
          ...
          steps={[
            {
              title: "Diagnose",
              body: "One to two weeks embedded with the team owning the problem. Paid discovery. Deliverable is the shape of the engagement, not the pitch.",
            },
            {
              title: "Contract",
              body: "Fixed-fee proposal with a schedule, a team, and the exact people. Signed in writing. Never billed by the hour.",
            },
            {
              title: "Ship",
              body: "Weekly demos and written updates on Fridays. Feature-flagged rollouts. No surprises at review.",
            },
            {
              title: "Steward",
              body: "We hand back a codebase, a brand, or a growth engine — with a team ready to own it, or a retainer to keep steering.",
            },
          ]}
        />
      </Section>
        ...
        <Container>
          <div className="grid grid-cols-1 gap-x-8 gap-y-10 md:grid-cols-12">
            <Reveal as="div" className="md:col-span-4">
              ...
              <h2 className="h2 mt-4 max-w-[14ch]">The people who run the engagements.</h2>
              <p className="lede mt-6 max-w-[42ch]">
                Six partners, forty engineers, designers, editors, and strategists. No junior
                handoffs.
              </p>
              <Link href="/careers" className="link-underline kicker mt-6 inline-block uppercase">
                Join the studio →
              </Link>
            </Reveal>
            <Stagger className="grid grid-cols-1 gap-x-8 gap-y-8 sm:grid-cols-2 md:col-span-8">
              {partners.map((p) => (
                <StaggerItem key={p.name}>
                  <div className="border-t border-[var(--color-line-strong)] pt-6">
                    ...
                    <p className="kicker mt-3">{p.location}</p>
                  </div>
                </StaggerItem>
              ))}
            </Stagger>
      ...
      <Section size="sm" className="border-t border-[var(--color-line)]">
        <Container wide>
          <span className="kicker mb-8 block">Selected clients</span>
          <LogoMarquee />
        </Container>
      </Section>
